...
export default function LoginPage() {
  ...
  const comingFromCheckout = callbackUrl.startsWith('/checkout');
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!result || result.error) {
      setError('Incorrect email or password. Use demo account below.');
      return;
    }
        ...
        <ScrollReveal className="w-full max-w-sm space-y-6 rounded-3xl border border-slate-200/70 bg-white/80 p-6 shadow-soft">
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-slate-500">
              Use the email and password you signed up with to view and track
              your Cash on Delivery orders.
            </p>
          </div>
